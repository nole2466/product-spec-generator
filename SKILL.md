...
| 角色 | 核心職責 | 主要產出 | 詳細指南 |
|-----|---------|---------|---------|
| **Scrum Master** | Sprint 協調、規格流轉 | Sprint 概覽、合併檢查 | [→ agents/scrum-master.md](agents/scrum-master.md) |
...
| **PD** | 需求轉規格、設計體驗 | Spec / Delta Spec | [→ agents/pd.md](agents/pd.md) |
...
產品需求文件/ ──→ 開發中/Sprint-X/ ──→ 功能規格/
    (PRD)           (delta.md)        (Source of Truth)
```

| 類型 | 動作 | 說明 |
|:---:|------|------|
| 🆕 新功能 | 移動 | `開發中/Sprint-X/{功能}/` → `功能規格/{功能}/` |
| 🔄 變更 | 合併 | delta.md 內容合併到現有 `規格.md` |

**詳細流程**：[→ core/sprint-workflow.md](core/sprint-workflow.md)
...
| 角色 | 說明 | 快速指令 |
|-----|------|---------|
| **Scrum Master** | 我要協調 Sprint、管理規格流轉 | `開始 Sprint-X` / `準備合併 Sprint-X` |
| **Coordinator** | 我要協調專案、追蹤進度 | `開始新功能：{功能名稱}` |
| **PM** | 我要提需求、管理 Backlog | `幫我撰寫「{功能}」的 PRD` |
| **PD** | 我要把需求轉成規格 + 設計 | `幫我撰寫「{功能}」的規格` / `幫我撰寫「{功能}」的 Delta Spec` |
